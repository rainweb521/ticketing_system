...
UserInfoDlg.collectData = function () {
    this.set('id').set('account').set('sex').set('password').set('avatar')
        .set('email').set('name').set('birthday').set('rePassword').set('deptid').set('phone').set('balance');
};
 ...
 * 余额充值
 */
UserInfoDlg.balanceSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/mgr/user_balance", function (data) {
        Feng.success("充值成功!");
        if (window.parent.MgrUser != undefined) {
            window.parent.MgrUser.table.refresh();
            UserInfoDlg.close();
