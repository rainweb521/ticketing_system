...
public class UserMgrController extends BaseController {
     ...
     * 跳转到余额充值页面
     */
    @RequestMapping(value = "/user_balance/{userId}",method = RequestMethod.GET)
    public String userBalance(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
        User user = this.userService.selectById(userId);
        model.addAttribute(user);
        return PREFIX + "user_balance.html";
    }
     ...
     * @throws NoPermissionException
     */
    @RequestMapping(value = "/user_balance",method = RequestMethod.POST)
    @ResponseBody
    public ResponseData userBalance(Integer id, Double balance)  {
        User oldUser = userService.selectById(id);
        if (oldUser.getBalance() == null) {
            oldUser.setBalance(0.0);
        }
